import React, { useEffect, useState } from 'react';
import { View, Text, TouchableOpacity, Animated, Modal } from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { SafeAreaView } from 'react-native-safe-area-context';
import { clearSnack, subscribe } from 'lib/snackbarService';
import { useTheme } from 'constants/useTheme';

const Snackbar: React.FC = () => {
    const { colors } = useTheme();
    const [snack, setSnack] = useState<any | null>(null);
    // start off-screen above
    const translateY = React.useRef(new Animated.Value(-120)).current;

    const dismissSnack = React.useCallback(() => {
        Animated.timing(translateY, { toValue: -120, duration: 200, useNativeDriver: true }).start(() => {
            clearSnack();
            setSnack(null);
        });
    }, [translateY]);

    useEffect(() => subscribe((s) => setSnack(s)), []);

    useEffect(() => {
        if (snack) {
            // settle the snack a bit lower so it's clearly inside the safe area
            Animated.spring(translateY, { toValue: 48, useNativeDriver: true }).start();
            const t = setTimeout(() => {
                dismissSnack();
            }, snack.duration || 4000);
            return () => clearTimeout(t);
        }
    }, [dismissSnack, snack, translateY]);

    if (!snack) return null;

    return (
        <Modal visible transparent animationType="none" statusBarTranslucent onRequestClose={dismissSnack}>
            <Animated.View style={{ position: 'absolute', left: 0, right: 0, top: 0, transform: [{ translateY }], zIndex: 9999 }} pointerEvents="box-none">
                <SafeAreaView style={{ paddingHorizontal: 12 }}>
                    <View style={{ backgroundColor: colors.primary, padding: 12, borderRadius: 8, flexDirection: 'row', alignItems: 'center', elevation: 6 }}>
                        <TouchableOpacity
                            onPress={dismissSnack}
                            accessibilityRole="button"
                            accessibilityLabel="Dismiss notification"
                            style={{ paddingRight: 10, paddingVertical: 4 }}
                        >
                            <Ionicons name="close" size={18} color={colors.onPrimary} />
                        </TouchableOpacity>
                        <Text style={{ color: colors.onPrimary, flex: 1 }}>{snack.message}</Text>
                        {snack.actionTitle ? (
                            <TouchableOpacity onPress={() => { snack.onAction && snack.onAction(); dismissSnack(); }} style={{ paddingLeft: 12, paddingVertical: 4 }}>
                                <Text style={{ color: colors.onPrimary, fontWeight: '600' }}>{snack.actionTitle}</Text>
                            </TouchableOpacity>
                        ) : null}
                    </View>
                </SafeAreaView>
            </Animated.View>
        </Modal>
    );
};

export default Snackbar;
